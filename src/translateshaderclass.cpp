...
bool TranslateShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"translateVS.hlsl", L"translatePS.hlsl");
	if (!result)
	{
		return false;
	}
...
bool TranslateShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
...
bool TranslateShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = device->CreateBuffer(&translateBufferDesc, 0, &m_translateBuffer);
	if (FAILED(result))
	{
		return false;
	}
